...
void fill_joystick_info(SDL_Joystick* joy, uint32_t id, rrjoy::JoystickInfoPtr joy_info)
{
    ...
    {
        joy_info->joystick_capabilities |=  rrjoy::JoystickCapabilities::standard_gamepad;
    }
        ...
        if (haptic)
        {
            ...
            if (haptic_query)
            {
                if (SDL_IsGameController(id))
                {
                    ...
                    {
                        joy_info->joystick_capabilities |=  rrjoy::JoystickCapabilities::rumble;
                    }
                    else
                    {   
                        if ((haptic_query & (SDL_HAPTIC_CARTESIAN)) != 0)
                        {
                            joy_info->joystick_capabilities |=  rrjoy::JoystickCapabilities::force_feedback;
                        }
                    }
                }
            }
...
rrjoy::GamepadStatePtr fill_gamepad_state(SDL_GameController* joy)
{
    ...
    joy_state->right_x = SDL_GameControllerGetAxis(joy, SDL_CONTROLLER_AXIS_RIGHTX);
    joy_state->right_y = SDL_GameControllerGetAxis(joy, SDL_CONTROLLER_AXIS_RIGHTY);
    joy_state->trigger_left = SDL_GameControllerGetAxis(joy, SDL_CONTROLLER_AXIS_TRIGGERLEFT);
    joy_state->trigger_right = SDL_GameControllerGetAxis(joy, SDL_CONTROLLER_AXIS_TRIGGERRIGHT);
...
void JoystickImpl::Open(uint32_t id, com::robotraconteur::hid::joystick::JoystickInfoPtr joy_info)
{
    ...
    if (SDL_JoystickIsHaptic(joy))
    {
        ...
        if (haptic)
        {
            ...
            if (SDL_HapticEffectSupported(haptic, (SDL_HapticEffect*)&this->constant_effect))
            {   
                this->constant_effect.type = SDL_HAPTIC_CONSTANT;
                this->constant_effect.length = 1000;
                this->constant_effect.level = 100;
                this->constant_effect_id = SDL_HapticNewEffect(haptic, (SDL_HapticEffect*)&this->constant_effect);
                this->has_ff = true;
            }
        }
    }
...
void JoystickImpl::rumble(double intensity, double duration)
{
    ...
    if (has_rumble)
    {
        SDL_HapticRumblePlay(this->haptic, (float)intensity, boost::numeric_cast<uint32_t>(duration*1000.0));
    }
}

void JoystickImpl::force_feedback(const com::robotraconteur::geometry::Vector2& force, double duration)
{
    boost::mutex::scoped_lock lock(this_lock);
    if (has_ff)
    {
        ...
        this->constant_effect.direction.dir[0] = boost::numeric_cast<int32_t>(force.s.x*10000.0);
        this->constant_effect.direction.dir[1] = boost::numeric_cast<int32_t>(force.s.y*10000.0);
        this->constant_effect.length = boost::numeric_cast<uint32_t>(duration*1000.0);
        this->constant_effect.level = boost::numeric_cast<int16_t>(sqrt(pow(force.s.x,2.0) + pow(force.s.y,2.0)) * boost::numeric_cast<double>(std::numeric_limits<int16_t>::max()));
        if(SDL_HapticUpdateEffect(haptic, this->constant_effect_id, (SDL_HapticEffect*)&this->constant_effect)!=0)
        {
...
{
    uint32_t local_ep = RR::ServerEndpoint::GetCurrentEndpoint()->GetLocalEndpoint();
    if (downsampler)
    {
...
double JoystickImpl::get_update_rate()
{
    return 100.0;
}
